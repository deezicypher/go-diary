...
func main() {
	v := Vertex{3, 4}
	v.Scale(10)
	v.ScaleF(&V, 10) //By passing the address of v, you allow the Scale function to directly modify the original Vertex struct. If you passed v by value (without the &), a copy of v would be passed to Scale, and the changes made inside the function would not affect the original
	fmt.Println(v.Abs())
